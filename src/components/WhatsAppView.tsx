...
import { Users, ChartBar, MessageSquare, Table, TrendingDown } from "lucide-react";
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <MetricsCard
          title="DSAT Score"
          value={`${processedData.dsatScore.toFixed(1)}%`}
          subtitle="Insatisfação geral"
          icon={<TrendingDown size={18} />}
          trend={processedData.dsatScore > 0 ? { value: 5, isPositive: false } : undefined}
        />
        <MetricsCard
          title="Melhor Grupo"
          value={processedData.bestGroup}
          subtitle={`DSAT: ${processedData.bestGroupDsat}%`}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <div className="text-sm text-gray-500 dark:text-gray-400">
                      {group.totalRatings} avaliações • DSAT: {group.dsat}%
                    </div>
                  </div>
                </div>
        ...
        {processedData.totalRatings > 0 && (
          <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl shadow-sm p-6">
            <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-6">Análise DSAT</h3>
            <div className="space-y-4">
              <div className="p-4 bg-green-50 dark:bg-green-900/20 rounded-lg border border-green-200 dark:border-green-800">
                <div className="text-2xl font-bold text-green-700 dark:text-green-400">
                  {Math.round((processedData.positiveRatings / processedData.totalRatings) * 100)}%
                </div>
                <div className="text-sm font-medium text-green-700 dark:text-green-400 mt-1">Avaliações Positivas</div>
                <div className="text-xs text-green-600 dark:text-green-500">😀 😀 🤩 ({processedData.positiveRatings} avaliações)</div>
              </div>
              
              <div className="p-4 bg-yellow-50 dark:bg-yellow-900/20 rounded-lg border border-yellow-200 dark:border-yellow-800">
                <div className="text-2xl font-bold text-yellow-700 dark:text-yellow-400">
                  {Math.round(((processedData.totalRatings - processedData.positiveRatings - processedData.negativeRatings) / processedData.totalRatings) * 100)}%
                </div>
                <div className="text-sm font-medium text-yellow-700 dark:text-yellow-400 mt-1">Avaliações Neutras</div>
                <div className="text-xs text-yellow-600 dark:text-yellow-500">😐 🙁 (notas 2-3)</div>
              </div>
              
              <div className="p-4 bg-red-50 dark:bg-red-900/20 rounded-lg border border-red-200 dark:border-red-800">
                <div className="text-2xl font-bold text-red-700 dark:text-red-400">
                  {processedData.dsatScore.toFixed(1)}%
                </div>
                <div className="text-sm font-medium text-red-700 dark:text-red-400 mt-1">DSAT Score</div>
